...
const TEACHING_FILE = path.join(DATA_DIR, 'runtime-teaching-authority.json');
...
function getTeachingContinuity(userId, limit = 30) {
  const store = readStore();
  return (store[userId] || []).slice(-limit);
}

function buildTeachingAuthorityContext(userId) {
  const continuity = getTeachingContinuity(userId, 75);
  ...
  return {
    ...
    guidance: {
      ...
      maintainGenesisToRevelationContinuity: true,
      preserveScriptureFirstApproach: true,
      avoidDenominationalSynthesis: true,
    },
  };
}
...
module.exports = {
  saveTeachingStudy,
  getTeachingContinuity,
  buildTeachingAuthorityContext,
};
