#!/usr/bin/env node
/**
 * Phase 2I — post-2H full conversation stress re-run (105 scenarios / 125 turns).
 * Usage: export OPENAI_API_KEY=sk-... && node scripts/phase2iConversationStressTest.js
 */
...
const OUT = path.join(__dirname, '..', 'docs', 'regression-trace', 'phase2i-conversation-stress-results.json');
...
async function runTurn({ scenarioId, group, userId, message, turnIndex = 0 }) {
  ...
  return {
    ...
    memoryDeltaMb: (memAfter.rssMB ?? memAfter.rssMb ?? 0) - (memBefore.rssMB ?? memBefore.rssMb ?? 0),
    latencyMs,
    runtimeError,
