...
function main() {
  ...
  assert.ok(
    includes(review.moderationRules, 'Do not auto-publish unreviewed materials.'),
    'unreviewed resources must not auto-publish'
  );
  assert.ok(
    includes(review.moderationRules, 'Do not treat AI review as final authority.'),
    'AI review must not become final authority'
  );
  assert.ok(
    includes(review.moderationRules, 'Require human approval before knowledge ingestion.'),
    'human approval must remain mandatory before knowledge ingestion'
  );
  assert.strictEqual(
    review.aiReviewOutputs && review.aiReviewOutputs.approvalRecommendation,
    'human_review_required',
    'AI output must continue to defer final approval to a human reviewer'
  );
  assert.strictEqual(
    review.alignmentSeverity && review.alignmentSeverity.highRiskTeaching && review.alignmentSeverity.highRiskTeaching.action,
    'lock_from_ingestion_until_human_approval',
    'high-risk teaching material must stay locked until human approval'
  );
